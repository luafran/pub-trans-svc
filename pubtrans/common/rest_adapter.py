...
from pubtrans.common import enable_curl_rest_adapter
...
class RestAdapter(object):
    ...
    def __init__(self, endpoint, context, support, validate_certs=None, certs=None,
        ...
        self._validate_certs = (validate_certs if validate_certs is not None else False)
        self._certs = certs
    ...
    def put(self, path=None, query=None, headers=None, body=None, timeout=None):
        """
    ...
    def _request(self, method, path, query, headers, body, timeout):
        ...
        try:
            response = yield self._http_client.fetch(request, raise_error=False)
