...
class CircuitBreaker(object):  # pylint: disable=too-many-instance-attributes
    """A single circuit with breaker logic."""

    # --pylint: disable=too-many-arguments
    def __init__(self, clock, log, error_types, maxfail,
                 reset_timeout, time_unit):
        self.clock = clock
        self.log = log
...
class CircuitBreakerSet(object):  # pylint: disable=too-many-instance-attributes
    ...
    def context(self, circuit_id):
        ...
        if circuit_id not in self.circuits:
            self.circuits[circuit_id] = self.factory(self.clock,
                                                     self.log.getChild(circuit_id),
                                                     self.error_types,
                                                     self.maxfail,
                                                     self.reset_timeout,
                                                     self.time_unit)
